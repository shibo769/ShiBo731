...
### By Random Sum of Random Variable ###
SN_mean = N_mean * X_mean
SN_var = N_mean * X_var + (X_mean)**2 * N_var
SN_skew = (X_mean_cube) / sqrt(Lambda*(X_mean_sq)**3)
### Gamma methods params ###
gamma_alpha = 4 * (SN_skew)**(-2)
gamma_beta = sqrt(gamma_alpha / (Lambda * X_mean_sq))
gamma_k = Lambda * X_mean - gamma_alpha / gamma_beta
...
### Poisson Draws ###
Ns = np.random.poisson(Lambda, M)
### Log-normal Draws ###
Xs = [np.random.lognormal(mu, sigma, N) for N in Ns]
### Compound ###
SNs = sorted(np.array([np.sum(X) for X in Xs]))
### GP ###
low_point = np.quantile(SNs, alpha0, interpolation='higher')
high_point = np.quantile(SNs, 0.99999, interpolation='higher')
u = np.quantile(SNs, alpha0, interpolation='higher')
SNu = pd.Series(SNs)
SN_exceed = (SNu[SNu>u] - u).values 
shape, loc, scale = gp.fit(SN_exceed, floc=0)
### cdf values
cdf_xasix = np.linspace(low_point, high_point, 1000)
G_cdf = lambda x: 1 - (1 + shape*x/scale)**(-1/shape)
gp_approx = lambda x: G_cdf(x - u)*(1 - alpha0) + alpha0
### Emp ###
N_emp = np.random.poisson(Lambda, M)
X_emp = [np.random.lognormal(mu, sigma, N) for N in Ns]
SNs_emp = sorted(np.array([np.sum(X) for X in Xs]))

low_point_ind = SNs.index(low_point)
high_point_ind = SNs.index(high_point)

emp_len = len(SNs[low_point_ind: high_point_ind+1])
cdf_emp = np.linspace(low_point_ind, high_point_ind, emp_len) / M

### Plots ###
plt.plot(SNs[low_point_ind: high_point_ind+1], 1 - cdf_emp)
...
plt.xscale("log")
plt.yscale("log")
plt.legend(['Empirical','Normal','GP','Gamma'])
plt.title("Log-log plot for Approximating Compound Poisson", fontsize = 13)
plt.show()

def ES_gamma(a, b, alpha):
    temp = a / b + gamma.ppf(alpha,a,1/b) * gamma.pdf(gamma.ppf(alpha,a,1/b), a,1/b)  / (b * (1-alpha))
    return temp

seq_alpha = np.arange(alpha0, 0.99999, 0.00001)
gammaES = ES_gamma(gamma_alpha, gamma_beta, seq_alpha)
...
plt.figure(dpi = 100)
plt.plot(seq_alpha, gammaES,label = "gamma")
plt.plot(seq_alpha, GPES,label = "GP")
plt.plot(seq_alpha, normalES,label = "Normal")
plt.xlabel("alpha")
plt.ylabel("ES-alpha Sn")
plt.title("ES for Sn")
plt.legend()
plt.show()
