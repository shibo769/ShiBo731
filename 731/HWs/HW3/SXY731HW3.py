...
for i in range(M, N):
    ...
    mu = Lambda * mu + (1 - Lambda) * X.iloc[i]
    temp = (np.array(X.iloc[i] - mu).reshape(5,1))*(np.array(X.iloc[i] - mu).reshape(1,5))
    ...
    ### Spectral
    sp_data[i+1-M] = -positions[i].dot(mu) + ((positions[i].dot(cov.dot(positions[i])))**0.5)*Spectral_Shit
    ### Marginal Spectral
    mar_sp_data[i+1-M] = -mu + np.array(Spectral_Shit)*(np.transpose(cov.dot(positions[i])))/((positions[i].dot(np.transpose(cov.dot(positions[i]))))**0.5)
    ### Components Spectral
